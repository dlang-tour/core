...
void main()
{
    ...
    static assert(!__traits(compiles, forever = 5;));
    writeln("forever: ",
        typeof(forever).stringof);

    const int* cForever = &forever;
    static assert(!__traits(compiles, *cForever = 10;));
    ...
    static assert(!__traits(compiles, *cMutable = 100;));
    writeln("cMutable: ",
        typeof(cMutable).stringof);

    static assert(!__traits(compiles, immutable int* imMutable = &mutable;));
}
